...
def sender_plot():
    ...
    plt.plot(x_smooth, y2_smooth, 'r', marker='*', label="medium QoS level",
             markersize=4)

    y3_smooth = spline(x, y3, x_smooth)
    plt.plot(x_smooth, y3_smooth, color='g', marker='x', markersize=4,
             label="low QoS level")

    y4_smooth = spline(x, y4, x_smooth)
    plt.plot(x_smooth, y4_smooth, color='k', marker='o', markersize=4,
             label="best effort")
